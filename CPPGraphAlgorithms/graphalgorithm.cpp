...
vector<vertex_id> top_sort(const Graph * const graph) {
	...
	for(auto it = vertices.begin(); it != vertices.edn(); ++it) {
		in_degrees[*it]=0;
	}
	...
	while(!zero_indegree_nodes.empty()) {
		...
		for(auto it = adjacent.begin(); it != adjacent.edn(); ++it) {
			--in_degrees[*it];
			if(in_degrees[*it] == 0) {
	...
	for(size_t i = 1; i <= num_vertices; ++i) {
		if(in_degrees[i] > 0) {
			// Returning a vector with nothing inside signifies a topological sort is not possible
			order.clear();
			break;
		}
	}
